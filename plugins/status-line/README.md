...
up the existing one first), seeds a starter config, and shows a preview. Run
`/reload-plugins` after installing so Claude Code exposes `/status-line:setup`.
Restart Claude Code if the new status line does not appear immediately after
setup.
...
- Run `/reload-plugins` after plugin updates so Claude Code sees the latest
  commands, then re-run `/status-line:setup` to refresh the stable runtime under
  `$CLAUDE_CONFIG_DIR/plugins/status-line/.venv`.
